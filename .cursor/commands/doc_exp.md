...
- ``config_files/rl/config_default.yaml`` (or the YAML used for the run): ``training`` section (batch_size, learning rates, schedules), ``performance`` (running_speed, gpu_collectors_count), ``environment`` (if env params changed), ``nn`` (if architecture changed: ``vis``, ``decoder``, ``iqn``, ``fusion_mode``, etc.)  
