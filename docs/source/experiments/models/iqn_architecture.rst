...
- **Optional multi-action offsets** — if ``environment.rl_action_offsets_ms`` contains multiple values, one decision predicts a block of future actions for offsets such as ``[0, 10, 20, 30, 40]`` ms. Internally this is controlled by ``n_actions_per_block``.
...
1. **Collectors** — Several game instances run in parallel. Each has an **inference_network** (copy of the policy). The agent observes (frame, float_state), chooses either a single action or a block of actions (ε-greedy or Boltzmann over mean Q), and sends it to the game. In multi-action mode one forward pass predicts ``N`` actions for the configured offsets. Transitions (state, action, reward, next_state, …) are sent to the learner via a queue.
...
In multi-action mode the target is factorized over offsets: we predict ``N`` action heads per decision block and bootstrap with the **sum over offsets**. With DDQN enabled, the online network chooses the best action independently at each offset and the target network evaluates that block. Without DDQN, the target network directly uses ``sum_i max_a Q_i(s', a)``.
...
This network does **distributional** RL: it models the *distribution* of the return (sum of rewards), not just its expectation. Standard DQN outputs one Q(s,a) = E[return]; here we predict **quantiles** of that distribution — for each τ ∈ (0,1) we get the τ-quantile (e.g. τ=0.1 = pessimistic, τ=0.5 = median). We get K values per state-action pair; averaging them gives the usual Q estimate, but the full set captures uncertainty and often improves learning (IQN, QR-DQN, C51 are distributional methods).
...
**Replication (“repeating” state K times)** — We have one state representation after concat, shape (B, D). For each state we need Q for K different τ. So we *repeat* that representation K times → (B×K, D), and for each of the B×K rows we compute a τ-dependent embedding and mix it (Hadamard) with the repeated state. The result is (B×K, D): one row per (state, quantile). The A and V heads then output Q of shape ``(B×K, n_actions)`` in single-action mode, or ``(B×K, N, n_actions)`` in multi-action mode. So “replication” is: one state → K rows (one per τ) so we get K quantile estimates per state in one forward pass.
...
- **Q** — When ``n_actions_per_block = 1``: shape ``(batch_size * num_quantiles, n_actions)``. When ``n_actions_per_block > 1``: shape ``(batch_size * num_quantiles, n_actions_per_block, n_actions)``.
- **tau** — The quantiles used: shape ``(batch_size * num_quantiles, 1)``.

The network uses a **dueling** layout: from a single shared representation it computes value V and advantages A, then Q = V + A - mean(A). In multi-action mode the scalar value stream is broadcast across the ``N`` offset heads.
...
- **Dueling** — From this representation the advantage head A and value head V are computed. In single-action mode this yields one action vector; in multi-action mode it yields ``N`` action vectors, one per future offset (details below).
...
Single-action mode (``n_actions_per_block = 1``):
...
- Then: ``Q = V + A - mean(A, dim=actions)``.

Multi-action mode (``n_actions_per_block = N > 1``):

- **A_head** becomes a shared hidden projection: Linear(D → dense_hidden_dimension//2) → LeakyReLU.
- **A_head_multi** is a single fused linear layer producing ``N * n_actions`` logits, then reshaped to ``(B×K, N, n_actions)``.
- **V_head** still predicts ``(B×K, 1)`` and is broadcast across offsets.
- Then: ``Q = V.unsqueeze(1) + A - mean(A, dim=actions)``.

This means the current multi-action architecture is **factorized over offsets**: the network predicts one action distribution per future slot and the learner sums the selected slot-values to obtain the block value used in TD learning.
...
Config parameter ``dense_hidden_dimension`` (e.g. 1024); then the inner layer of the heads is 512. The final layers of ``A_head`` / ``A_head_multi`` and ``V_head`` are initialized orthogonally without extra gain.

Multi-action offsets and decision blocks
----------------------------------------

When ``environment.rl_action_offsets_ms`` has more than one value, the project switches from "one decision = one action" to "one decision = one action block".

Example:

- ``rl_action_offsets_ms = [0, 10, 20, 30, 40]``
- ``n_actions_per_block = 5``

Interpretation:

- at each decision point the agent predicts 5 discrete actions,
- each predicted action corresponds to a different future offset,
- the rollout applies these actions on the underlying 10 ms cadence,
- replay stores one transition per block, not one transition per individual 10 ms action.

In this mode:

- the network output is structured as ``(quantile, offset, action_class)``,
- exploration can be configured per action or per whole block,
- TD learning uses the sum of the chosen offset-values as the value of the block.
...
- **Exploration** — At inference we use ε-greedy or Boltzmann over the **mean** of the K quantile outputs (config: ``exploration`` section). In single-action mode this gives one scalar Q per action. In multi-action mode it gives one scalar Q table of shape ``(N, n_actions)`` and exploration can be applied either independently per offset or jointly for the whole block.
...
- **rl_action_offsets_ms** — List of future offsets; if its length is greater than 1, the architecture uses ``n_actions_per_block = len(rl_action_offsets_ms)`` and activates the multi-action output path.
