...
- **A01_long_v3**: Another **single-map A01** long run. TensorBoard logs: ``tensorboard\A01_long_v3``. At documentation time the logged data in that dir was minimal (no scalar metrics, single race tag with very short duration); use the same analysis script to compare with A01_as20_long or other long runs when more data is available.
